...
werkzeug.urls.url_decode = werkzeug.urls.url_unquote_plus   # fallback
...
# ---------- Backup Endpoint (called by cron-job.org) ----------
...
# This runs inside the app context and is executed only once when the first request comes in.
# Using with app.app_context() ensures the database tables are ready.
def create_admin():
...
# Call the function to ensure admin exists (safe to call at startup)
create_admin()
